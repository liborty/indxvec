use core::{
    ...
    /// the range (the last low bound .. the last high bound).
    ...
    /// Search within the specified RangeInclusive<T> index, which is always ascending.
        ...
        // f64:From<T>, usize:From<T>, u128:From<T>
    {
        ...
        // Now lo and hi are never equal to target
        // Binary search for first match
        let (mid, rrange) = binary_any(range,cmpr);
        let (lor, _) = binary_any(&(rrange.start..mid),&mut |&probe| downend(cmpr(&probe)));
        let (hir, _) = binary_any(& (mid..rrange.end),&mut |&probe| upend(cmpr(&probe)));
        lor+T::from(1)..hir+T::from(1)
    }